...
const validate = (xmlDocs) => {
	...
	const allCuriosityReferences = xmlDocs.map((doc, index) => [...doc.getElementsByTagName("Curiosity")].map(element => { return {element, referenceID: element.innerHTML, index} }))
	allCuriosityReferences
		.map(reference => 
			getElementByIDTag(allEntryIDs, referenceID, `An <Entry> in file ${reference.index} has an invalid reference ID for <Curiosity>: "${reference.referenceID}". There is no <Entry> with the supplied ID.`)
...
const validateReferences = (referencerType, possibleReferencees, referenceeType) => {
	const allReferences = xmlDocs.map((doc, index) => [...doc.getElementsByTagName("SourceID")].map(element => { return {element, referenceID: element.innerHTML, index} }))
	allReferences
		.map(reference => 
...
const getElementByIDTag = (idsList, id, errMsg) => {
	const possibleElements = idsList.filter(idMeta => idMeta.id === id).map(idMeta.element)
