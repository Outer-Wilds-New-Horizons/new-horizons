...
namespace NewHorizons.Utility.DebugUtilities
{
    ...
    internal class DebugPropPlacer : MonoBehaviour
    {
        ...
        // VASE
        public static readonly string DEFAULT_OBJECT =
            "BrittleHollow_Body/Sector_BH/Sector_NorthHemisphere/Sector_NorthPole/Sector_HangingCity/Sector_HangingCity_District1/Props_HangingCity_District1/OtherComponentsGroup/Props_HangingCity_Building_10/Prefab_NOM_VaseThin";

        public string currentObject { get; private set; } // path of the prop to be placed
        private bool hasAddedCurrentObjectToRecentsList;
        private readonly List<PropPlacementData> props = new List<PropPlacementData>();
        private readonly List<PropPlacementData> deletedProps = new List<PropPlacementData>();
        ...
        private void Update()
        {
            ...
            if (Keyboard.current[Key.Minus].wasReleasedThisFrame) DeleteLast();

            if (Keyboard.current[Key.Equals].wasReleasedThisFrame) UndoDelete();
        }
        ...
        {
            var data = _rc.Raycast();
            PlaceObject(data, gameObject.transform.position);

            if (!hasAddedCurrentObjectToRecentsList)
            {
                hasAddedCurrentObjectToRecentsList = true;

                if (!RecentlyPlacedProps.Contains(currentObject)) RecentlyPlacedProps.Add(currentObject);
            }
        }
        ...
        public void PlaceObject(DebugRaycastData data, Vector3 playerAbsolutePosition)
        {
            ...
            try
            {
                if (currentObject == "" || currentObject == null) SetCurrentObject(DEFAULT_OBJECT);

                var prop = DetailBuilder.MakeDetail(data.hitObject, data.hitObject.GetComponentInChildren<Sector>(),
                    currentObject, data.pos, data.norm, 1, false);
                var propData = RegisterProp_WithReturn(data.bodyName, prop);

                // align with surface normal
                var alignToSurface =
                    (Quaternion.LookRotation(data.norm) * Quaternion.FromToRotation(Vector3.up, Vector3.forward))
                    .eulerAngles;
                ...
                var dirTowardsPlayer = prop.transform.parent.transform.InverseTransformPoint(playerAbsolutePosition) -
                                       prop.transform.localPosition;
                dirTowardsPlayer.y = 0;
                var rotation = Quaternion.LookRotation(dirTowardsPlayer).eulerAngles.y;
            ...
            {
                Logger.Log(
                    $"Failed to place object {currentObject} on body ${data.hitObject} at location ${data.pos}.");
            }
        }
        ...
        public static string GetAstroObjectName(string bodyName)
        {
            ...
            if (astroObjectName.EndsWith("_Body"))
                astroObjectName = astroObjectName.Substring(0, astroObjectName.Length - "_Body".Length);
        ...
        public void FindAndRegisterPropsFromConfig(PlanetConfig config)
        {
            ...
            foreach (var detail in config.Props.details)
            {
                ...
                var data = RegisterProp_WithReturn(astroObjectName, spawnedProp, detail.path, detail);
        ...
        private PropPlacementData RegisterProp_WithReturn(string bodyGameObjectName, GameObject prop,
            string propPath = null, DetailInfo detailInfo = null)
        {
            if (Main.Debug)
            {
            ...
            var bodyName = GetAstroObjectName(bodyGameObjectName);
            ...
            var data = new PropPlacementData
            {
                body = bodyName,
                gameObject = prop,
        ...
        public Dictionary<string, DetailInfo[]> GetPropsConfigByBody()
        {
            ...
            var propConfigs = new Dictionary<string, DetailInfo[]>();

            foreach (var bodyProps in groupedProps)
            {
                ...
                var bodyName = GetAstroObjectName(bodyProps[0].body);

                var infoArray = new DetailInfo[bodyProps.Count];
                ...
                for (var i = 0; i < bodyProps.Count; i++)
                {
                    ...
                    if (!prop.detailInfo.alignToNormal)
                        prop.detailInfo.rotation = rootTransform
                            .InverseTransformRotation(prop.gameObject.transform.rotation).eulerAngles;
        ...
        public void UndoDelete()
        {
            ...
            var last = deletedProps[deletedProps.Count - 1];
