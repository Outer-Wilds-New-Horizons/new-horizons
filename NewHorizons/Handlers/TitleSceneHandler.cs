...
namespace NewHorizons.Handlers
{
    public static class TitleSceneHandler
    {
        ...
        {
            var subtitleContainer = GameObject.Find("TitleMenu/TitleCanvas/TitleLayoutGroup/Logo_EchoesOfTheEye");
        ...
        public static void DisplayBodyOnTitleScreen(List<NewHorizonsBody> bodies)
        {
            ...
            var eligible = bodies.Where(b =>
                (b.Config.HeightMap != null || b.Config.Atmosphere?.clouds != null) && b.Config.Star == null &&
                b.Config.canShowOnTitle).ToArray();
        ...
        private static GameObject LoadTitleScreenBody(NewHorizonsBody body)
        {
            ...
            var heightMap = new HeightMapModule();
            var minSize = 15;
            var maxSize = 30;
            float size = minSize;
            ...
            var pivot = Object.Instantiate(GameObject.Find("Scene/Background/PlanetPivot"),
                GameObject.Find("Scene/Background").transform);
            pivot.GetComponent<RotateTransform>()._degreesPerSecond = 10f;
            foreach (Transform child in pivot.transform) Object.Destroy(child.gameObject);
            pivot.name = "Pivot";

            if (body.Config.Ring != null)
            {
                var newRing = new RingModule();
