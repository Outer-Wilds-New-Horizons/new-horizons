...
namespace NewHorizons.Patches
{
    ...
    public class RaftPatches : HarmonyPatch
    {
        ...
        public static bool RaftController_FixedUpdate(RaftController __instance)
        {
            ...
            if (__instance._raftBody.IsSuspended()) return false;
            var playerInEffectsRange = __instance._playerInEffectsRange;
            __instance._playerInEffectsRange =
                (Locator.GetPlayerBody().GetPosition() - __instance._raftBody.GetPosition()).sqrMagnitude < 2500f;
            ...
            if (__instance._dock != null || __instance._movingToTarget)
            {
                ...
                if (__instance._movingToTarget) __instance.UpdateMoveToTarget();
                return false;
            }
            ...
            if (__instance._playerInEffectsRange)
            {
                ...
                var num = __instance._fluidDetector.InFluidType(FluidVolume.Type.WATER)
                    ? __instance._fluidDetector._alignmentFluid.GetFractionSubmerged(__instance._fluidDetector)
                    : 0f;
                var allowMovement = num > 0.25f && num < 1f;
        ...
        public static bool AlignToSurfaceFluidDetector_ManagedFixedUpdate(AlignToSurfaceFluidDetector __instance)
        {
            ...
            if (__instance._inAlignmentFluid)
            {
                ...
                var alignmentDirection = (__instance.transform.position - __instance._alignmentFluid.transform.position)
                    .normalized;
                ...
                var a = OWPhysics.FromToAngularVelocity(currentDirection, alignmentDirection);
