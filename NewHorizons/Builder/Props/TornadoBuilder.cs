        ...
        public static void Make(GameObject planetGO, Sector sector, PropModule.TornadoInfo info, bool hasClouds)
        {
            if (_upPrefab == null)
            {
                _upPrefab = GameObject
                    .Find(
                        "BrittleHollow_Body/Sector_BH/Sector_SouthHemisphere/Sector_SouthPole/Sector_Observatory/Interactables_Observatory/MockUpTornado")
                    .InstantiateInactive();
            ...
            if (_downPrefab == null)
            {
                _downPrefab = GameObject
                    .Find(
                        "BrittleHollow_Body/Sector_BH/Sector_SouthHemisphere/Sector_SouthPole/Sector_Observatory/Interactables_Observatory/MockDownTornado")
                    .InstantiateInactive();
            ...
            {
                _hurricanePrefab = GameObject
                    .Find("GiantsDeep_Body/Sector_GD/Sector_GDInterior/Tornadoes_GDInterior/Hurricane/")
                    .InstantiateInactive();
            ...
            {
                _soundPrefab = GameObject
                    .Find(
                        "GiantsDeep_Body/Sector_GD/Sector_GDInterior/Tornadoes_GDInterior/SouthernTornadoes/DownTornado_Pivot/DownTornado/AudioRail")
                    .InstantiateInactive();
            ...
            if (info.type == PropModule.TornadoInfo.TornadoType.Hurricane)
                MakeHurricane(planetGO, sector, info, position, hasClouds);
            else
                MakeTornado(planetGO, sector, info, position,
                    info.type == PropModule.TornadoInfo.TornadoType.Downwards);
        }

        private static void MakeTornado(GameObject planetGO, Sector sector, PropModule.TornadoInfo info,
            Vector3 position, bool downwards)
        {
            ...
            tornadoGO.transform.rotation =
                Quaternion.FromToRotation(Vector3.up, sector.transform.TransformDirection(position.normalized));
            ...
            if (info.tint != null) ApplyTint(tornadoGO, info.tint, false, downwards);

            if (info.wanderRate != 0) ApplyWanderer(tornadoGO, sector, info);

            soundGO.SetActive(true);
            tornadoGO.SetActive(true);
        }

        private static void MakeHurricane(GameObject planetGO, Sector sector, PropModule.TornadoInfo info,
            Vector3 position, bool hasClouds)
        {
            ...
            hurricaneGO.transform.rotation =
                Quaternion.FromToRotation(Vector3.up, sector.transform.TransformDirection(position.normalized));
            ...
            foreach (var rotate in hurricaneGO.GetComponentsInChildren<RotateTransform>()) rotate._sector = sector;
            ...
            if (info.tint != null) ApplyTint(hurricaneGO, info.tint, true, false);

            if (info.wanderRate != 0) ApplyWanderer(hurricaneGO, sector, info);
